...
function KnowledgeBaseSvg({ navIndex, activeNavIndex, theme }) {
  const color = `${navIndex === activeNavIndex && theme !== 'dark' ? '#6837EF' : navIndex === activeNavIndex && theme === 'dark' ? '#CBB2F5' : theme === 'dark' ? '#B8B4C0' : '#696D8C'}`

  return (
    <svg className={`transition-transform duration-200 ease-in-out`} width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
